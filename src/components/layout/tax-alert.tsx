...
export function TaxAlert() {
  ...
  useEffect(() => {
    ...
    let dismissed = false;
    try { dismissed = localStorage.getItem(`lv-erp-tax-alert-dismissed:${monthKey}`) === '1'; } catch {}
    if (dismissed) return;
    let cancelled = false;
    (async () => {
      try {
        ...
        const dl = upcomingTaxDeadlines(today, switchYm);
        if (!dl.length) return;
        ...
        if (!cancelled && out.length) { setLines(out); setOpen(true); }
      } catch {}
    })();
  ...
  return (
    <AppDialog open={open} onClose={() => dismiss(false)} title="세금 마감이 다가옵니다" description="시트 저장값으로 계산한 예상 금액입니다. 정확한 금액은 세무사와 확인하세요.">
