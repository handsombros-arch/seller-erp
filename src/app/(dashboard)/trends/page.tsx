...
function formatPeriod(period: string, unit: Unit) {
  if (unit === 'week') return period; // 2026-W12
  if (unit === 'month') return period; // 2026-03
  return period.slice(5); // 03-22
}
...
export default function TrendsPage() {
  ...
  return (
    <div className="space-y-6">
      {/* 헤더 */}
      <div>
        <h1 className="text-[20px] font-bold text-[#191F28] tracking-[-0.02em]">재고 · 판매 추이</h1>
        <p className="text-[13px] text-[#6B7684] mt-1">쿠팡 그로스 + 자사 창고 재고와 판매량을 한눈에 비교합니다.</p>
      </div>
        ...
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height={380}>
            <ComposedChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
              ...
              <XAxis
                dataKey="period"
                tickFormatter={(v) => formatPeriod(v, unit)}
              ...
              <Tooltip
                ...
                formatter={((value: number, name: string) => {
                  const labels: Record<string, string> = {
                    coupang_qty: '쿠팡 재고', warehouse_qty: '창고 재고', sales_qty: '판매량',
                  };
                  return [formatNumber(value) + '개', labels[name] ?? name];
                }) as any}
                labelFormatter={(label) => formatPeriod(String(label), unit)}
              />
              <Legend
