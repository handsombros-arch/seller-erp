...
interface LineDraft { key: string; id?: string; sku_id: string | null; display_name: string; qty: number; unit_cost: number; unit_price: number; note: string }
...
export const b2bTotals = (lines: { qty: number; unit_cost: number; unit_price: number }[]): B2bTotals => ({
  qty: lines.reduce((s, l) => s + (Number(l.qty) || 0), 0),
  count: lines.length,
  supply: lines.reduce((s, l) => s + (Number(l.qty) || 0) * (Number(l.unit_price) || 0), 0),
...
interface B2bLineApi { id: string; sku_id: string | null; display_name: string; qty: number; unit_cost: number; unit_price: number; note: string | null }
const toDraft = (l: B2bLineApi): LineDraft => ({ key: l.id, id: l.id, sku_id: l.sku_id, display_name: l.display_name, qty: Number(l.qty) || 0, unit_cost: Number(l.unit_cost) || 0, unit_price: Number(l.unit_price) || 0, note: l.note ?? '' });
...
export function B2bLinesDialog({ open, onClose, ym, readOnly, onSaved }: { open: boolean; onClose: () => void; ym: string; readOnly: boolean; onSaved?: (t: B2bTotals) => void }) {
  ...
  const add = (skuId: string) => {
    ...
    setLines(prev => {
      ...
      return [...prev, { key: `new:${Date.now()}:${prev.length}`, sku_id: skuId, display_name: skuLabel(s), qty: 1, unit_cost: Number(s.cost_price) || 0, unit_price: 0, note: '' }];
    });
    setDirty(true);
  };
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const r = await fetch('/api/settlement/b2b-lines', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ year_month: ym, lines: lines.map(l => ({ id: l.id, sku_id: l.sku_id, display_name: l.display_name, qty: l.qty, unit_cost: l.unit_cost, unit_price: l.unit_price, note: l.note })) }) });
  ...
  return (
    <AppDialog open={open} onClose={async () => { if (saving) return; if (dirty && !(await confirmDialog('저장하지 않은 변경이 있습니다. 닫을까요?'))) return; onClose(); }} title={`B2B 출고 내역 · ${ymLabel(ym)}`} wide className="max-w-4xl"
      description="마스터 SKU 를 고르면 줄이 추가됩니다 (같은 SKU 는 수량 +1). 공급단가는 세금계산서 공급가액 기준(VAT 별도), 원가는 마스터 값이 들어오고 고칠 수 있습니다. 합계는 시트 B2B 매출·매입원가의 기준값이 되고 재고는 바뀌지 않습니다.">
          ...
          <table className="w-full text-[12px]">
            <thead>
              <tr className="text-fg-4 text-[11px] border-b border-line-2">
                ...
                <th className="text-right px-2 py-1.5 font-medium w-28">공급단가</th>
            ...
            <tbody>
              {lines.map(l => (
                <tr key={l.key} className="border-b border-line-2/60">
                  ...
                  <td className="px-1 py-1"><input type="text" inputMode="numeric" disabled={readOnly} value={l.unit_price ? fmtNum(l.unit_price) : ''} placeholder="0" onChange={e => update(l.key, { unit_price: num(e.target.value) })} className={cn(inputClassName, cell, 'w-full', !l.unit_price && 'border-warn')} /></td>
                  ...
                  <td className="px-2 py-1 text-right tabular-nums text-fg">{fmtNum(l.qty * l.unit_price)}</td>
