...
interface Sku { id: string; code: string; name: string; productId: string | null; costPrice?: number | null; coupangVid?: string | null; extraVids?: string[] }
...
export function UnregisteredCard({ selectedYm, onRegistered }: { selectedYm: string; onRegistered?: () => void }) {
  ...
  async function register(it: Item) {
    ...
    // 같은 SKU 에 이미 다른 쿠팡 옵션ID 가 있으면(윙/그로스 두 리스팅) 덮어쓰지 않고 "추가 옵션ID" 로 등록한다
    const chosen = data.skus.find(s => s.id === f.skuId);
    const asExtra = !!chosen?.coupangVid && chosen.coupangVid !== it.vendorItemId;
    setBusy(it.vendorItemId);
    try {
      if (asExtra) {
        const r = await fetch('/api/platform-sku-ids', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sku_id: f.skuId, channel_id: data.coupangChannelId, platform_sku_id: it.vendorItemId, label: '추가', price: f.price ? Number(f.price.replace(/[^0-9]/g, '')) : null }) });
        const j = await r.json().catch(() => ({}));
        if (!r.ok) { toast.error(`추가 옵션ID 등록 실패: ${j.error ?? r.status}`); return; }
      } else {
      ...
      toast.success(asExtra ? `${it.vendorItemId} 을 ${chosen!.code} 의 추가 옵션ID 로 등록 (기존 ${chosen!.coupangVid} 유지)` : `${it.vendorItemId} 등록 — 다음 집계부터 자동 매칭됩니다`);
      load(); onRegistered?.();
    ...
    <section className="rounded-2xl border border-warn/40 bg-warn/5 px-4 md:px-5 py-3">
      ...
      {open && (
        <div className="mt-3 space-y-2">
          <p className="text-[11px] text-fg-4">SKU 연결만 필수. 원가·판매가·수수료율은 비워도 됩니다(원가는 SKU 값 유지, 수수료는 기본 정책). 이미 옵션ID 가 있는 SKU 를 고르면 덮어쓰지 않고 추가 옵션ID 로 붙습니다(윙·그로스 두 리스팅). 잘못 연결했으면 마스터 시트 쿠팡 열의 옵션ID 칸(쉼표로 여러 개)에서 고치면 됩니다.</p>
            ...
            return (
              <div key={it.vendorItemId} className="flex flex-wrap items-center gap-2 rounded-xl bg-card px-3 py-2 border border-line">
                ...
                <div className="ml-auto flex items-center gap-1.5">
                  ...
                  {overwrite && <span className="text-[10px] text-brand whitespace-nowrap" title={`이 SKU 는 이미 쿠팡 옵션ID ${prevVid} 에 연결됨 — 윙/그로스처럼 리스팅이 두 개면 둘 다 유지됩니다`}>기존 {prevVid} 유지 + 추가 ID</span>}
