    ...
    <section className="bg-card rounded-2xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] px-4 md:px-5 py-4">
      <div className="flex flex-wrap items-center gap-2">
        <button onClick={() => setOpen(v => !v)} className="flex items-center gap-1.5 text-left mr-auto">
          ...
          <span className="text-[12px] text-fg-3">달마다 무엇이 들어와 있는지 · 빠진 것은 주황</span>
          <span className="text-[11px] text-fg-4 ml-1">{open ? '접기' : '펼치기'}</span>
        </button>
            ...
            <div className="overflow-x-auto rounded-xl border border-line">
              <table className="w-full text-[12px] border-collapse">
                <thead><tr className="h-9 bg-card-2 border-b border-line text-[11px] font-semibold text-fg-4">
                  ...
                  <th className={cell}>시트</th>
                  {SALES_COLS.map(c => <th key={c.id} className={cell} title="매출 파일 적용 행 수 / 미매칭">{c.label}</th>)}
                  <th className={cell} title="monthly_product_ads 월 집계 (광고비 합)">쿠팡 광고</th>
                  <th className={cell} title="토스 raw 에 있는 날짜 수 / 그 달 일수 · 기간">토스 광고</th>
                  <th className={cell}>B2B</th>
                </tr></thead>
                <tbody>
                  {months.map(m => { const c = data.coverage[m]; const sel = m === selectedYm; const t = c?.tossAds; const tossFull = t && t.days >= t.daysInMonth * 0.9;
                    return <tr key={m} className={cn('h-9 border-b border-line-2', sel && 'bg-brand-bg/50')}>
                      ...
                      {SALES_COLS.map(col => { const s = c?.sales[col.id]; return <td key={col.id} className={cn(cell, s ? (s.unmatched ? part : ok) : none)} title={s ? `${s.rows}행 · ${fmtNum(s.qty)}개 · 매출 ${fmtNum(s.revenue)}원${s.unmatched ? ` · 미매칭 ${s.unmatched}행` : ''}${s.updated_at ? ` · ${s.updated_at.slice(0, 10)}` : ''}` : '매출 파일 없음'}>{s ? (s.unmatched ? `${s.rows}행 (미매칭 ${s.unmatched})` : `${s.rows}행`) : '–'}</td>; })}
                      <td className={cn(cell, c?.coupangAds ? ok : pcDays[m] ? part : none)} title={c?.coupangAds ? `월 집계 ${c.coupangAds.rows}옵션 · 광고비 ${fmtNum(c.coupangAds.cost)}원 (VAT 별도)${pcDays[m] ? ` · 이 PC raw ${pcDays[m]}일치` : ''}` : pcDays[m] ? `이 PC 에 raw ${pcDays[m]}일치 — 월 집계 저장 필요` : '광고 분석 › 쿠팡에 보고서 없음'}>{c?.coupangAds ? `${fmtNum(c.coupangAds.cost)}${pcDays[m] ? ` · ${pcDays[m]}일` : ''}` : pcDays[m] ? `PC ${pcDays[m]}일` : '–'}</td>
                      <td className={cn(cell, t ? (tossFull ? ok : part) : none)} title={t ? `raw ${t.rawRows}행 · ${t.first ?? ''}~${t.last ?? ''} · 광고비 ${fmtNum(t.cost)}원${t.aggregated ? ' · 월 집계 ✓' : ' · 월 집계 없음(재저장 필요)'}` : '광고 분석 › 토스에 보고서 없음'}>{t ? `${t.days}/${t.daysInMonth}일` : '–'}</td>
