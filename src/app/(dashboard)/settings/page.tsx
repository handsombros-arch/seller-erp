    ...
    <section>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-[13px] font-bold text-fg">창고 관리</h2>
        </div>
        <button
          onClick={openAdd}
          className="flex items-center gap-1 px-3.5 py-1.5 bg-brand text-white text-[13px] font-medium rounded-xl hover:bg-brand-hover transition-colors"
        >
      ...
      <div className="bg-card rounded-2xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] overflow-hidden">
        {loading ? (
          ...
          <div className="py-10 text-center text-[13px] text-fg-5">등록된 창고가 없습니다</div>
        ) : (
          <div className="divide-y divide-line-2">
            {warehouses.map((w) => (
              <div key={w.id} className="flex items-center justify-between px-5 py-4 hover:bg-card-2 transition-colors">
                <div>
                  <div className="flex items-center gap-2">
                    <p className="text-[13px] font-medium text-fg">{w.name}</p>
                    <span className={`inline-block px-2 py-0.5 rounded-lg text-[11px] font-medium ${WAREHOUSE_TYPE_BADGE[w.type] ?? 'bg-card-2 text-fg-3'}`}>
                      {WAREHOUSE_TYPE_LABELS[w.type] ?? w.type}
                    </span>
                    {!w.is_active && (
                      <span className="inline-block px-2 py-0.5 rounded-lg text-[11px] font-medium bg-card-2 text-fg-3">비활성</span>
                    )}
                  </div>
                  {w.location && (
                    <p className="text-[12px] text-fg-5 mt-0.5">{w.location}</p>
                  )}
                </div>
                <button
                  onClick={() => openEdit(w)}
                  className="flex items-center gap-1 px-3 py-1.5 text-[12px] text-fg-3 bg-app rounded-xl hover:bg-line transition-colors"
                >
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="bg-card rounded-2xl max-w-sm p-0 gap-0 overflow-hidden">
          <DialogHeader className="px-6 pt-6 pb-4 border-b border-line-2">
            <DialogTitle className="text-[15px] font-bold text-fg">
          ...
          <form onSubmit={handleSubmit}>
            <div className="px-6 py-5 space-y-4">
              <div>
                <label className="block text-[13px] font-medium text-fg mb-1.5">창고명 *</label>
                <input
                  type="text"
                  value={form.name}
                  ...
                  required
                  placeholder="창고명 입력"
                  className="w-full px-3 py-2 text-[13px] border border-line rounded-xl focus:outline-none focus:ring-2 focus:ring-brand"
                />
              </div>
              <div>
                <label className="block text-[13px] font-medium text-fg mb-1.5">유형 *</label>
                <select
                  value={form.type}
                  onChange={(e) => setForm((f) => ({ ...f, type: e.target.value }))}
                  className="w-full px-3 py-2 text-[13px] border border-line rounded-xl focus:outline-none focus:ring-2 focus:ring-brand bg-card"
                >
              ...
              <div>
                <label className="block text-[13px] font-medium text-fg mb-1.5">위치</label>
                <input
                  type="text"
                  value={form.location}
            ...
            <DialogFooter className="px-6 pb-6 pt-0 gap-2">
              <button
                ...
                className="flex-1 px-4 py-2.5 text-[13px] font-medium text-fg-3 bg-app rounded-xl hover:bg-line transition-colors"
              >
                취소
              </button>
              <button
                type="submit"
                disabled={submitting}
                className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 text-[13px] font-medium text-white bg-brand rounded-xl hover:bg-brand-hover transition-colors disabled:opacity-60"
              >
    ...
    <section>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-[13px] font-bold text-fg">채널 관리</h2>
        </div>
        <button
          onClick={openAdd}
          className="flex items-center gap-1 px-3.5 py-1.5 bg-brand text-white text-[13px] font-medium rounded-xl hover:bg-brand-hover transition-colors"
      ...
      <div className="bg-card rounded-2xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] overflow-hidden">
        {loading ? (
          ...
          <div className="py-10 text-center text-[13px] text-fg-5">등록된 채널이 없습니다</div>
        ) : (
          <div className="divide-y divide-line-2">
            {channels.map((c) => (
              <div key={c.id} className="flex items-center justify-between px-5 py-4 hover:bg-card-2 transition-colors">
                <div className="flex items-center gap-3">
                  <span className={`inline-block px-2 py-0.5 rounded-lg text-[11px] font-medium ${CHANNEL_TYPE_BADGE[c.type] ?? 'bg-card-2 text-fg-3'}`}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="bg-card rounded-2xl max-w-sm p-0 gap-0 overflow-hidden">
          <DialogHeader className="px-6 pt-6 pb-4 border-b border-line-2">
            <DialogTitle className="text-[15px] font-bold text-fg">{editTarget ? '채널 수정' : '채널 추가'}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit}>
            <div className="px-6 py-5 space-y-4">
              <div>
                <label className="block text-[13px] font-medium text-fg mb-1.5">채널명 *</label>
                <input
                  type="text"
                  value={form.name}
                  onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-[13px] font-bold text-fg">계정 정보</h2>
      </div>

      <div className="bg-card rounded-2xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] overflow-hidden divide-y divide-line-2">
        {/* Email row */}
        <div className="flex items-center justify-between px-5 py-4">
          <div>
            <p className="text-[12px] text-fg-3 mb-0.5">이메일</p>
            <p className="text-[13px] font-medium text-fg">{email || '-'}</p>
          </div>
        </div>
        ...
        <div className="px-5 py-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[12px] text-fg-3 mb-0.5">비밀번호</p>
              <p className="text-[13px] text-fg">••••••••</p>
            </div>
            <button
              onClick={() => { setShowPwForm((v) => !v); setPwMsg(null); }}
              className="px-3.5 py-1.5 text-[13px] font-medium text-brand bg-brand-bg rounded-xl hover:bg-brand-bg transition-colors"
            >
              {showPwForm ? '취소' : '변경'}
            </button>
          </div>
          ...
          {showPwForm && (
            <form onSubmit={handlePasswordChange} className="mt-4 space-y-3">
              <div>
                <label className="block text-[12px] font-medium text-fg-3 mb-1">새 비밀번호</label>
                <input
                  type="password"
                  value={pwForm.next}
              ...
              <div>
                <label className="block text-[12px] font-medium text-fg-3 mb-1">새 비밀번호 확인</label>
                <input
                  type="password"
              ...
              <button
                type="submit"
                disabled={pwLoading}
                className="flex items-center justify-center gap-1.5 w-full py-2.5 text-[13px] font-medium text-white bg-brand rounded-xl hover:bg-brand-hover transition-colors disabled:opacity-60"
              >
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-[13px] font-bold text-fg">쿠팡 Open API 연동</h2>
      </div>
      ...
      <div className="bg-card rounded-2xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] overflow-hidden divide-y divide-line-2">
          ...
          <div className="px-5 py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              {connected ? (
                ...
                <AlertCircle className="w-5 h-5 text-fg-5 shrink-0" />
              )}
              <div>
                {connected ? (
                  <>
                    <p className="text-[13px] font-medium text-fg">연동됨 · Vendor ID: {connected.vendor_id}</p>
                  ...
                  <>
                    <p className="text-[13px] font-medium text-fg">API 키 미등록</p>
                    <p className="text-[12px] text-fg-5 mt-0.5">쿠팡 Wing &gt; 개발자 센터에서 API 키를 발급받으세요</p>
                  </>
                )}
              </div>
            </div>
        ...
        {connected && !showForm && (
          <div className="px-5 py-4 flex items-center justify-between">
            <div>
              <p className="text-[13px] font-medium text-fg">로켓그로스 세이버</p>
              <p className="text-[12px] text-fg-5 mt-0.5">월 99,000원(VAT별도) · 반품회수비/재입고비 면제</p>
            </div>
            <button
              ...
              className={`px-4 py-2 rounded-xl text-[13px] font-semibold transition-all active:scale-95 ${
                rgSaverEnabled ? 'bg-brand text-white ring-2 ring-brand/30' : 'bg-app text-fg-3 hover:bg-line'
              }`}>
        ...
        {showForm && (
          <form onSubmit={handleSave} className="px-5 py-5 space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="block text-[13px] font-medium text-fg">Access Key *</label>
            ...
            <div className="space-y-1.5">
              <label className="block text-[13px] font-medium text-fg">Secret Key *</label>
            ...
            <div className="space-y-1.5">
              <label className="block text-[13px] font-medium text-fg">Vendor ID (업체코드) *</label>
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-[13px] font-bold text-fg">네이버 커머스 API 연동</h2>
      </div>
      <div className="bg-card rounded-2xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] overflow-hidden divide-y divide-line-2">
        {loading ? (
          <div className="flex items-center justify-center py-10">
          ...
          <div className="px-5 py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                {connected ? (
                  <>
                    <p className="text-[13px] font-medium text-fg">연동됨 · Client ID: {connected.client_id}</p>
                    <p className="text-[12px] text-fg-5 mt-0.5">마지막 업데이트 {new Date(connected.updated_at).toLocaleDateString('ko-KR')}</p>
                  </>
                ) : (
                  <>
                    <p className="text-[13px] font-medium text-fg">API 키 미등록</p>
                    <p className="text-[12px] text-fg-5 mt-0.5">네이버 스마트스토어센터 → 판매자 정보 → API 연동에서 발급</p>
                  </>
                )}
              </div>
            </div>
        ...
        {showForm && (
          <form onSubmit={handleSave} className="px-5 py-5 space-y-4">
            <div className="space-y-1.5">
              <label className="block text-[13px] font-medium text-fg">애플리케이션 ID (Client ID) *</label>
            ...
            <div className="space-y-1.5">
              <label className="block text-[13px] font-medium text-fg">Client Secret *</label>
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-[13px] font-bold text-fg">데이터 초기화</h2>
      </div>
          ...
          {!confirm ? (
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              <button
                ...
                className="px-4 py-2 text-[13px] font-medium text-fg-3 bg-app rounded-xl hover:bg-line transition-colors"
              >
                취소
              </button>
            </div>
...
const SYNC_CHANNELS = [
  ...
  { key: 'toss', label: '토스', color: 'bg-brand-bg text-brand-hover' },
] as const;
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-3">
        <RefreshCw className="w-5 h-5 text-fg" />
        <h2 className="text-[15px] font-bold text-fg">수동 동기화</h2>
      </div>
      <div className="bg-card rounded-2xl border border-line p-5">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 text-[13px] font-semibold text-white bg-fg rounded-xl hover:bg-fg disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
          ...
          {SYNC_CHANNELS.map((ch) => (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 text-[12px] font-medium rounded-xl border border-line hover:bg-card-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors`}
            >
        ...
        {results && (
          <div className="space-y-1.5 p-3 bg-card-2 rounded-xl text-[12px]">
            {results.error && (
...
export default function SettingsPage() {
  return (
    <div className="space-y-8">
      <div>
        <PageHeader title="설정" />
        <p className="text-[13px] text-fg-3 mt-0.5">창고, 채널, 계정을 관리합니다</p>
      </div>
      <ManualSyncSection />
      <WarehouseSection />
      <ChannelSection />
