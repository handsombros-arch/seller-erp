...
function ScoreBar({ score, max = 10 }: { score: number; max?: number }) {
  ...
  const color = score >= 8 ? 'bg-green-500' : score >= 6 ? 'bg-brand' : score >= 4 ? 'bg-yellow-500' : 'bg-red-500';
  return (
...
export default function SourcingDetailPage() {
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
        ...
        <div className="bg-card border rounded-lg p-5">
          <div className="flex gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <PageHeader title={data.product_info?.title || '(분석 중...)'} />
              <a href={data.url} target="_blank" rel="noopener noreferrer" className="text-xs text-brand hover:underline break-all" title={data.url}>{data.url}</a>
            </div>
          </div>
          ...
          {data.status !== 'done' && data.status !== 'failed' && (
            <div className="mt-4">
              <div className="flex items-center justify-between text-xs mb-1">
                ...
                <span className="text-brand-hover font-bold">{pct}%</span>
              </div>
          ...
          {data.product_info && (
            <div className="flex items-end gap-4 mt-4 flex-wrap">
              {data.product_info.finalPrice && (
                <div>
                  <div className="text-xs text-fg-3 mb-0.5">최종가 (실 결제)</div>
                  <div className="text-3xl font-bold text-brand">{data.product_info.finalPrice}</div>
                </div>
              )}
              {data.product_info.salesPrice && data.product_info.salesPrice !== data.product_info.finalPrice && (
                <div>
                  <div className="text-xs text-fg-3 mb-0.5">판매가</div>
                  <div className="text-lg font-semibold text-fg-2">{data.product_info.salesPrice}</div>
                </div>
              )}
              {data.product_info.originalPrice && data.product_info.originalPrice !== data.product_info.finalPrice && (
                <div>
                  <div className="text-xs text-fg-3 mb-0.5">정가</div>
                  <div className="text-base text-fg-5 line-through">{data.product_info.originalPrice}</div>
                </div>
              )}
              {!data.product_info.finalPrice && data.product_info.price && (
                <div>
                  <div className="text-xs text-fg-3 mb-0.5">가격</div>
                  <div className="text-2xl font-bold text-brand">{data.product_info.price}</div>
                </div>
              )}
            </div>
          )}
          {data.review_stats && (
            <div className="flex gap-6 text-sm mt-4 pt-4 border-t flex-wrap items-center">
              <div>
                <span className="text-fg-3">리뷰:</span>{' '}
                ...
                {data.review_stats.officialReviewCount && data.review_stats.crawledCount &&
                  data.review_stats.officialReviewCount !== data.review_stats.crawledCount && (
                  <span className="ml-1 text-xs text-fg-5">(수집 {data.review_stats.crawledCount}개)</span>
                )}
              </div>
              <div>
                <span className="text-fg-3">평균:</span>{' '}
                <strong>{data.review_stats.officialAvgRating ?? data.review_stats.avgRating}★</strong>
              </div>
              <div><span className="text-fg-3">사진리뷰:</span> <strong>{data.review_stats.withImages || 0}</strong>개</div>
              {data.inquiries_count !== null && (
                <div><span className="text-fg-3">상품문의:</span> <strong>{data.inquiries_count}</strong>건</div>
              )}
            </div>
          )}
      ...
      {!hasAnalysis ? (
        <div className="text-center py-12 text-fg-3">분석 결과가 없습니다. 처리 완료 후 다시 확인하세요.</div>
      ) : (
        <>
          {/* ════ 2. 소싱 판단 ════ */}
          <Section id="verdict" title="소싱 판단" icon="🎯">
            {ra.summary_one_line && (
              <div className="bg-gradient-to-r from-blue-50 to-purple-50 border rounded-lg p-4">
                <div className="text-xs uppercase font-semibold text-fg-3 mb-1">한 줄 평</div>
                <p className="text-base font-semibold text-fg">{ra.summary_one_line}</p>
              </div>
            )}
            {sd.verdict && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {sd.differentiation_strategy && (
                    <div className="bg-white/70 rounded p-3">
                      <div className="text-xs uppercase font-semibold text-fg-3 mb-1">차별화 전략</div>
                      <div className="text-sm">{sd.differentiation_strategy}</div>
                    </div>
                  )}
                  {sd.key_risks?.length > 0 && (
                    <div className="bg-white/70 rounded p-3">
                      <div className="text-xs uppercase font-semibold text-fg-3 mb-1">핵심 리스크</div>
                  ...
                  {sd.recommended_actions?.length > 0 && (
                    <div className="bg-white/70 rounded p-3 md:col-span-2">
                      <div className="text-xs uppercase font-semibold text-fg-3 mb-1">권장 조치</div>
            ...
            {ra.summary_paragraph && (
              <div className="bg-card border rounded-lg p-4">
                <div className="text-xs uppercase font-semibold text-fg-3 mb-2">종합 평가</div>
                <p className="text-sm whitespace-pre-wrap leading-relaxed">{ra.summary_paragraph}</p>
              </div>
            )}
            {ra._sampling_info && (
              <details className="bg-card-2 border rounded-lg p-3 text-xs">
                <summary className="cursor-pointer font-semibold text-fg-3">📐 분석 객관성 정보 (샘플링 방식)</summary>
                <div className="mt-2 space-y-1 text-fg-3">
              ...
              <div className="bg-card border rounded-lg p-4 space-y-4">
                {dims.map((d: any, i: number) => (
                  <div key={i} className="pb-3 border-b last:border-0 last:pb-0">
                    ...
                    {d.spec_evidence && (
                      <div className="text-xs mt-2 bg-brand-bg rounded px-2 py-1 inline-block">
                        <strong className="text-brand-hover">📐 스펙:</strong> <span className="text-blue-900">{d.spec_evidence}</span>
                      </div>
                    )}
                    <div className="text-xs text-fg-3 mt-2 grid grid-cols-1 md:grid-cols-2 gap-2">
                      {d.page_claim && <div><strong className="text-fg-3">페이지 주장:</strong> {d.page_claim}</div>}
                      {d.review_consensus && <div><strong className="text-fg-3">리뷰 합의:</strong> {d.review_consensus}</div>}
                    </div>
                    {d.evidence && <div className="text-xs text-fg-2 mt-1 italic">{d.evidence}</div>}
                  </div>
                ))}
              </div>
            </Section>
          ...
          <Section id="pros-cons" title="장단점 분석" icon="⚖️">
                  ...
                  <ul className="space-y-3 text-sm">
                    {ra.pros_ranked.map((p: any, i: number) => (
                      <li key={i} className="border-l-2 border-green-200 pl-2">
                        ...
                        {p.category_significance && <div className="text-xs text-fg-3 mt-1 italic">{p.category_significance}</div>}
                      </li>
                    ))}
                  </ul>
                </div>
                  ...
                  <ul className="space-y-3 text-sm">
                    {ra.cons_ranked.map((p: any, i: number) => (
                      <li key={i} className="border-l-2 border-red-200 pl-2">
                        ...
                        <div className="font-medium">{p.point}</div>
                        {p.severity_reason && <div className="text-xs text-fg-3 mt-1 italic">왜 {p.severity}: {p.severity_reason}</div>}
                      </li>
                    ))}
                  </ul>
                </div>
                  ...
                  <ul className="space-y-2 text-sm">
                    {ra.hidden_weaknesses.map((h: any, i: number) => (
                      <li key={i}>
                        ...
                        {h.review_count_estimate && <div className="text-xs text-fg-3 ml-3">{h.review_count_estimate}</div>}
                      </li>
                    ))}
                  </ul>
                </div>
                  ...
                  <ul className="space-y-2 text-sm">
                    {ra.overpromised_claims.map((o: any, i: number) => (
                      <li key={i}>
                        ...
                        {o.review_evidence && <div className="text-xs text-fg-3 ml-3">{o.review_evidence}</div>}
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            {ra.neutral_observations?.length > 0 && (
              <div className="bg-card-2 border rounded-lg p-4">
                <h3 className="font-semibold mb-2">⚪ 중립적 관찰</h3>
                <p className="text-xs text-fg-3 mb-2">긍정/부정으로 단정 어려운 호불호 포인트</p>
              ...
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                ...
                <div className="space-y-3">
                  {ra.improvements_needed.map((imp: any, i: number) => (
                    <div key={i} className="bg-card rounded p-3 border-l-4 border-purple-400">
                      ...
                      {imp.current_state && <div className="text-xs text-fg-3 mt-1"><strong>현재:</strong> {imp.current_state}</div>}
                      {imp.suggested_change && <div className="text-sm text-purple-700 mt-1"><strong>🔧 개선:</strong> {imp.suggested_change}</div>}
                      {imp.impact_estimate && <div className="text-xs text-fg-3 mt-1"><strong>예상 효과:</strong> {imp.impact_estimate}</div>}
                    </div>
                  ))}
                </div>
              </div>
          ...
          {(ra.solvable_pain_points?.length > 0 || ra.differentiation_points?.length > 0 || ra.market_signals) && (
            <Section id="insights" title="차별화 인사이트" icon="💡">
              {ra.solvable_pain_points?.length > 0 && (
                <div className="bg-brand-bg border border-blue-200 rounded-lg p-4">
                  <h3 className="font-semibold mb-3 text-blue-800">해결 가능 포인트 (소싱 시 차별화 기회)</h3>
                  <div className="space-y-3">
                    {ra.solvable_pain_points.map((p: any, i: number) => (
                      <div key={i} className="bg-card rounded p-3 border-l-4 border-blue-400">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        {p.root_cause && <div className="text-xs text-fg-3 mt-1"><strong>원인:</strong> {p.root_cause}</div>}
                        {p.improvement_idea && <div className="text-sm text-brand-hover mt-1"><strong>💡 개선:</strong> {p.improvement_idea}</div>}
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                    <div className="border rounded p-2">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="text-xs text-fg-3">수요 강도</div>
                      ...
                      {ra.market_signals.demand_strength_reason && (
                        <div className="text-xs text-fg-3 italic">{ra.market_signals.demand_strength_reason}</div>
                      )}
                    </div>
                    <div className="border rounded p-2">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="text-xs text-fg-3">시장 포화</div>
                      ...
                      {ra.market_signals.saturation_risk_reason && (
                        <div className="text-xs text-fg-3 italic">{ra.market_signals.saturation_risk_reason}</div>
                      )}
                    </div>
                    <div className="border rounded p-2">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="text-xs text-fg-3">가격 포지션</div>
                        <Badge color="purple">{ra.market_signals.price_position}</Badge>
                      </div>
                      {ra.market_signals.price_position_reason && (
                        <div className="text-xs text-fg-3 italic">{ra.market_signals.price_position_reason}</div>
                      )}
                    </div>
                    <div className="border rounded p-2">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="text-xs text-fg-3">트렌드 지속</div>
                      ...
                      {ra.market_signals.trend_durability_reason && (
                        <div className="text-xs text-fg-3 italic">{ra.market_signals.trend_durability_reason}</div>
                      )}
                    </div>
                  </div>
                </div>
          ...
          {(ra.buyer_personas?.length > 0 || ra.use_case_distribution?.length > 0 || ra.options_feedback?.length > 0 || data.review_stats?.ratingDist || ra.sentiment_breakdown || ra.review_topic_frequency?.length > 0) && (
            <Section id="customers" title="고객 분석 · 통계" icon="👥">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {ra.neutral_score !== undefined && (
                  <div className="bg-card border rounded-lg p-4 text-center">
                    <div className="text-xs text-fg-3 mb-1">중립 종합 점수</div>
                    <div className={`text-4xl font-bold ${
                      ra.neutral_score >= 8 ? 'text-green-600' : ra.neutral_score >= 6 ? 'text-brand' : ra.neutral_score >= 4 ? 'text-yellow-600' : 'text-red-600'
                    }`}>{ra.neutral_score}</div>
                    <div className="text-xs text-fg-5">/ 10</div>
                    {ra.neutral_score_reasoning && <div className="text-xs text-fg-3 mt-2">{ra.neutral_score_reasoning}</div>}
                  </div>
                )}
                  ...
                  <div className="bg-card border rounded-lg p-4 md:col-span-2">
                    ...
                    <div className="flex h-8 rounded-lg overflow-hidden">
                      ...
                      {ra.sentiment_breakdown.neutral_pct > 0 && (
                        <div className="bg-fg-5 flex items-center justify-center text-white text-xs font-bold" style={{width: `${ra.sentiment_breakdown.neutral_pct}%`}}>
                    ...
                    {ra.sentiment_breakdown.notes && <div className="text-xs text-fg-3 mt-2">{ra.sentiment_breakdown.notes}</div>}
                  </div>
                )}
              </div>
                        ...
                        <div key={i}>
                          <div className="flex items-center justify-between mb-1">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs text-fg-3">{t.mention_count}건</span>
                            </div>
                            <span className="text-xs text-fg-3">{pct2}%</span>
                          </div>
                          <div className="bg-card-2 rounded h-2 overflow-hidden">
                            <div className={`${sentColor} h-full`} style={{width: `${pct2}%`}} />
                          </div>
                          {t.key_quotes?.length > 0 && (
                            <div className="text-xs text-fg-3 mt-1 italic ml-1">
              ...
              {ra.meaningful_keywords?.length > 0 && (
                <div className="bg-card border rounded-lg p-4">
                  <h3 className="font-semibold mb-3">🔑 리뷰 핵심 키워드 <span className="text-xs font-normal text-fg-3 ml-1">({ra.meaningful_keywords.length}개)</span></h3>
                  <div className="flex flex-wrap gap-2">
                    {ra.meaningful_keywords.map((k: any, i: number) => {
                      const sentBg = k.sentiment === 'positive' ? 'bg-green-50 border-green-300 text-green-800'
                        ...
                        : 'bg-card-2 border-line text-fg-2';
              ...
              {(ra.page_intended_targets?.length > 0 || ra.actual_buyer_targets?.length > 0) && (
                <div className="bg-card border rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      <div className="text-xs font-semibold text-brand-hover mb-2">📋 페이지 의도 타겟</div>
                      <ul className="space-y-2 text-sm">
                        {(ra.page_intended_targets || []).map((t: any, i: number) => (
                          <li key={i} className="bg-brand-bg rounded p-2">
                            <div><strong>{t.who}</strong></div>
                            {t.evidence && <div className="text-xs text-fg-3 mt-1 italic">근거: {t.evidence}</div>}
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {ra.target_match_analysis && (
                    <div className="mt-3 bg-card-2 rounded p-3 text-sm">
                      <strong className="text-xs text-fg-3">매칭 분석:</strong> {ra.target_match_analysis}
                    </div>
                  )}
                </div>
              )}
                    ...
                    <ul className="space-y-2 text-sm">
                      {ra.buyer_personas.map((b: any, i: number) => (
                        <li key={i} className="border-l-2 border-line pl-2">
                          ...
                          {b.buying_motivation && <div className="text-xs text-fg-3">동기: {b.buying_motivation}</div>}
                          {b.satisfaction_expected && <div className="text-xs text-fg-3">만족도: {b.satisfaction_expected}</div>}
                        </li>
                      ))}
                    </ul>
                  </div>
                    ...
                    {[5,4,3,2,1].map((s) => {
                      ...
                      return (
                        <div key={s} className="flex items-center gap-2 text-xs">
                          ...
                          <div className="w-20 text-right text-fg-3">{cnt}개 ({pct2}%)</div>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div className="bg-card border rounded-lg p-4 space-y-4 text-sm">
                      ...
                      <table className="w-full text-sm">
                        <tbody>
                          {Object.entries(da.specs).filter(([k, v]) => v != null && v !== '' && !k.startsWith('_')).map(([k, v], i) => (
                            <tr key={k} className={i % 2 === 0 ? 'bg-card-2' : 'bg-card'}>
                              <th className="text-left px-3 py-2 font-medium text-fg-3 w-32 align-top whitespace-nowrap">{k}</th>
                ...
                {da.target_demographics && (
                  <div className="pt-3 border-t">
                    ...
                    {da.target_demographics.primary && (
                      <div className="bg-brand-bg rounded p-2 mb-2">
                        ...
                        {typeof da.target_demographics.primary === 'object' && da.target_demographics.primary.evidence && (
                          <div className="text-xs text-fg-3 mt-1 italic">근거: {da.target_demographics.primary.evidence}</div>
                        )}
                      </div>
                    )}
                    {da.target_demographics.secondary?.length > 0 && (
                      <div className="space-y-1">
                        {da.target_demographics.secondary.map((s: any, i: number) => (
                          <div key={i} className="bg-card-2 rounded p-2">
                            <div><Badge color="gray">보조</Badge> {typeof s === 'string' ? s : s.who}</div>
                            {typeof s === 'object' && s.evidence && (
                              <div className="text-xs text-fg-3 mt-1 italic">근거: {s.evidence}</div>
                            )}
                          </div>
                        ))}
                      </div>
                ...
                {da.image_summary && (
                  <div className="pt-3 border-t">
                    ...
                    <p className="text-fg-2">{da.image_summary}</p>
                  </div>
                )}
            ...
            <Section id="inquiries" title={`상품문의 분석 (${data.inquiries_count || 0}건 · 가구매 영향 X)`} icon="❓">
              {ia.summary && (
                <div className="bg-card border rounded-lg p-4">
                  ...
                  {ia.answer_quality && <div className="text-xs text-fg-3 mt-2 italic">셀러 답변 품질: {ia.answer_quality}</div>}
                </div>
              )}
                  ...
                  <div className="space-y-2">
                    {ia.sourcing_insights.map((s: any, i: number) => (
                      <div key={i} className="bg-card rounded p-3 border-l-4 border-blue-400">
                        ...
                        {s.actionable && <div className="text-sm text-brand-hover mt-1">→ {s.actionable}</div>}
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <ul className="space-y-2 text-sm">
                    {ia.category_distribution.map((c: any, i: number) => (
                      <li key={i}>
                        ...
                        {c.examples?.length > 0 && (
                          <div className="text-xs text-fg-3 italic ml-2 mt-0.5">{c.examples.join(' / ')}</div>
                        )}
                      </li>
                    ))}
                  </ul>
