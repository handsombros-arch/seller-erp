...
[차원별 점수 rubric — 일관 적용 필수]
category_dimensions_scored 의 `score` (0~10점) 는 아래 기준으로 매기세요:

- 10: 카테고리 최상위. 스펙이 카테고리 평균을 크게 상회 + 리뷰 다수가 해당 차원을 강점으로 확정
- 8~9: 강점. 스펙 우수 + 리뷰 긍정 다수 + 치명 단점 없음
- 6~7: 보통. 경쟁력 있지만 차별화 약함. 스펙 평균 ± + 리뷰 혼재
- 4~5: 평균 이하. 명시적 약점 존재 (스펙 부족 또는 리뷰 다수 지적)
- 2~3: 치명적 약점. 부정 리뷰 다수 + 스펙 미달 → '치명적' severity 와 연동
- 0~1: 기능 실패 / 안전 문제 / 환불 사유

**점수 결정 원칙 (우선순위)**:
1) 객관 스펙이 카테고리 평균 대비 우위 → 기본 6~8점
2) 리뷰가 해당 차원을 긍정 다수로 확정 → +1~2점 가산
3) 리뷰가 부정 다수로 단점 지적 → -2~4점 감산 (severity 따라)
4) `spec_evidence` 가 null/미표기 면 리뷰 기반으로만 평가 (최대 6~7점 상한 권장 — 불확실성 반영)
5) `verdict` 와 일관되게: 강점→8+, 보통→6~7, 약점→4~5, 숨겨진단점→3~5, 치명적약점→0~2

**같은 카테고리 내 상품끼리 점수가 비교 가능해야 하므로, 카테고리 평균을 6점 기준선으로 삼고 차이를 벌리세요.**
